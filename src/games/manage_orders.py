...
def submit_order(name, channel, player, order, fromloc, toloc, targetloc, unit):
    ...
    reason = oeval.valid_order(gamedoc, gamedoc['currently_playing'][player], order, fromloc, toloc, targetloc, unit)
    if reason != 'valid':
    ...
    gamedoc['next_orders'] = [d for d in gamedoc['next_orders'] if d['from'] != fromloc]
    gamedoc['next_orders'].append(order)
