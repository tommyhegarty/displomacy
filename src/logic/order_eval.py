...
def valid_order(gamedoc, country, order, fromloc, toloc, targetloc, unit):
    state = gamedoc['state'][country]
    armies = state['armies']
    navies = state['fleets']
    ...
    if order == 'MOVE':
        ...
        if fromloc in navies and toloc in locked:
            return f'The unit at {fromloc} is a navy. It cannot move to landlocked provinces.'
